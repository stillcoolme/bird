...
如果要读 Buffer 中的值，需要切换模式，从写入模式切换到读出模式。注意，通常在说** NIO 的读操作，意思是从 Channel 中读数据到 Buffer 中，对应的是对 Buffer 的写入模式！**。

调用 Buffer 的 **flip()** 方法，可以从**写入模式切换到读取模式**。其实这个方法也就是设置了一下 position 和 limit 值罢了。
...
当然了，除了将数据从 Buffer 取出来使用，更常见的操作是将我们写入的数据传输到 Channel 中，如通过 FileChannel 将数据写入到文件中，通过 SocketChannel 将数据写入网络发送到远程机器等。对应的，这种操作，我们称之为**NIO 的 写操作**。
