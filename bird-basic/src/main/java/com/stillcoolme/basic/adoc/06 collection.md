...
* 在有大量数据写入 ArrayList 时，一定要初始化指定长度。
     1. ArrayList 的默认长度为 10；
     2. ArrayList是基于数组的，超过容量就对数组进行复制，复制到新开辟的1.5倍的内存空间，比较耗时。
* ArrayList的contains方法。
     1. 对数组进行for循环遍历，效率低。可以的话应该用Set来代替。
...
### 1.3 CopyOnWriteArrayList
 ...
 CopyOnWriteArrayList是一个基于 ReentrantLock  的**线程安全**的ArrayList ，通过写入时复制（CopyOnWrite，简称COW）思想（计算机程序设计领域中的一种优化策略）来设计，在修改时先复制出一个数组快照来修改，改好了，再让内部指针指向新数组：

> 如果有多个调用者（Callers）读取相同资源时，会共同获取相同的指针指向相同的资源。
>
> 当某个调用者视图修改资源内容时，系统会复制一份专用副本（private copy）给该调用者修改，而其他调用者得到的还是最初的资源仍然保持不变，直到新的修改完成。

因为对快照的修改对读操作来说不可见，所以读读之间不互斥，读写之间也不互斥，只有写写之间要加锁互斥。

CopyOnWrite的缺点　
主要存在两个问题，即内存占用问题和数据一致性问题。
